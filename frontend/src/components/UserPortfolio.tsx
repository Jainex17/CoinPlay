...
export const UserPortfolio = () => {
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-6 p-6 min-h-[60vh]">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground">
            The user <span className="text-red-400 font-mono">@{username}</span>{" "}
            doesn't exist or has no portfolio.
          </p>
        </div>
        <Link
          to="/"
  ...
  const { user, balance, claimed_cash, bets, coinHoldings } = publicPortfolio;
  ...
  const totalWagered =
    bets?.reduce((sum, bet) => sum + parseInt(bet.bet_amount), 0) || 0;
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
      ...
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription>
            Cryptocurrency holdings for @{user.username}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
              ...
              {coinHoldings && coinHoldings.length > 0 ? (
                coinHoldings.map((holding) => {
                  ...
                  const profitLossPercent =
                    totalSpent > 0
                      ? ((profitLoss / totalSpent) * 100).toFixed(2)
                      : "0.00";

                  return (
                    <TableRow key={holding.cid} className="group">
                      <TableCell>
                        <Link
                          to={`/coin/${holding.symbol}`}
                      ...
                      <TableCell>
                        <div className="flex flex-col">
                          <span
                            className={`font-mono font-semibold ${profitLoss >= 0 ? "text-green-400" : "text-red-400"}`}
                          >
                            {profitLoss >= 0 ? "+" : ""}
                            {formatCurrency(profitLoss)}
                          </span>
                          <span
                            className={`text-xs ${profitLoss >= 0 ? "text-green-400/70" : "text-red-400/70"}`}
                          >
                            ({profitLoss >= 0 ? "+" : ""}
                            {profitLossPercent}%)
                          </span>
                ...
                <TableRow>
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span>No coin holdings yet</span>
                    </div>
                  </TableCell>
                </TableRow>
              )}
      ...
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription>
            Betting history for @{user.username}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
              ...
              {bets && bets.length > 0 ? (
                bets.slice(0, 10).map((bet, index) => (
                  <TableRow key={index} className="group">
                    <TableCell className="font-mono">
                      {formatCurrency(parseInt(bet.bet_amount))}
                    </TableCell>
                    <TableCell>
                      <span

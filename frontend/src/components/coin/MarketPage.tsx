...
const MarketPage = () => {
    ...
    const totalCirculating = coins.reduce((acc, coin) => acc + coin.circulating_supply, 0);
            ...
            <div className="flex flex-col gap-6 px-4">
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
                    <div className="flex flex-col gap-2">
                        ...
                        <p className="text-muted-foreground font-medium">Explore and trade the hottest coins</p>
                    </div>
                </div>
                        ...
                        <Card className="bg-card/50 border-border shadow-xl rounded-xl p-6 hover:bg-muted/20 hover:scale-[1.02] transition-all duration-200 cursor-pointer group h-full">
                            ...
                            <div className="flex items-center gap-4 mb-5">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-lg font-black text-foreground group-hover:text-red-500 transition-colors">{coin.name}</span>
                                    <span className="text-sm text-muted-foreground font-medium">${coin.symbol}</span>
                                </div>
                            </div>
                            ...
                            <div className="flex items-end justify-between mb-5">
                                ...
                                <div className="flex flex-col items-end">
                                    ...
                                    <span className="text-sm font-bold text-foreground/80">@{coin.creator.username}</span>
                                </div>
                            </div>
                            ...
                            <div className="flex justify-between pt-4 border-t border-border">
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Total Supply</span>
                                    <span className="text-sm font-bold text-foreground/80">{formatNumber(coin.total_supply)}</span>
                                </div>
                                <div className="flex flex-col items-end">
                                    <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Circulating</span>
                                    <span className="text-sm font-bold text-foreground/80">{formatNumber(coin.circulating_supply)}</span>
                                </div>
                            </div>
                        </Card>
                    </Link>
                ...
                {coins.length === 0 && (
                    <Card className="bg-card/50 border-border shadow-xl rounded-xl p-12 text-center col-span-full">
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <p className="text-muted-foreground font-medium">No coins found</p>
                        </div>
                    </Card>
                )}
            </div>

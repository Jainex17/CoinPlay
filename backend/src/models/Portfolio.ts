...
export class PortfolioModel {
    ...
    static async findOrCreate(uid: number): Promise<Portfolio> {
        const client = await pool.connect();
        try {
            const result = await client.query(`SELECT * FROM portfolios WHERE uid = $1`, [uid]);
    ...
    static async updateClaim(uid: number, cash: number, CURRENT_TIMESTAMP: Date) {
        const client = await pool.connect();
        try {
            const result = await client.query(`UPDATE portfolios SET cash = cash + $1, last_claim_date = $2 WHERE uid = $3 RETURNING *`, [cash, CURRENT_TIMESTAMP, uid]);
            return result.rows[0];
        } catch (error) {
